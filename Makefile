...
DOCKER_RUN ?= docker run --rm -it --user "$(shell id -u):$(shell id -g)" -v ${CURDIR}:/opt/src --name qr-lipsync
...
ifeq (${VIDEO_FOUND}, 1)
	...
	${DOCKER_RUN} ${DOCKER_IMAGE} qr-lipsync-detect ${VIDEO} ${ARGS}
endif

generate:
	${DOCKER_RUN} ${DOCKER_IMAGE} qr-lipsync-generate --output-dir . ${ARGS}
